<template>
  <div class="flex flex-col w-screen min-h-screen">
    <nav class="sticky top-0 z-50 flex flex-row p-3 border border-l-0 border-r-0 border-t-0 border-gray-800 bg-gray-900 items-center justify-center">
      <div class="w-full max-w-6xl flex-row flex">
        <n-link to="/dashboard" aria-current="page" class="mr-auto flex items-center justify-center nuxt-link-exact-active nuxt-link-active">
        ...
        <ul class="w-full flex flex-row text-gray-300 text-sm items-center justify-end">
          <n-link to="/dashboard" aria-current="page" :class="get_active_styles('dashboard')">
            <li>Dashboard</li>
          </n-link>
          <n-link to="/dashboard/settings" :class="get_active_styles('dashboard-settings')">
            <li>Settings</li>
          </n-link>
          <n-link to="/logout" :class="get_active_styles('logout')">
            <li>Logout</li>
          </n-link>
        </ul>
      </div>
    ...
    <section class="flex flex-col items-center w-full bg-gray-200 min-h-full">
      <div style="min-height: calc(100vh - 64px);" class="flex flex-col max-w-6xl items-start justify-start w-11/12 bg-gray-100 border border-gray-300 border-t-0 border-b-0 relative">
        <Nuxt/>
      </div>
    </section>
  </div>
